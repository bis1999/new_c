...
oil_month_datasets = glob.glob('Data_monthly_oil/*.csv')
oil_mon_date = [i.split(".")[0].split("/")[1] for i in oil_month_datasets ]
oil_mon_date_max = max(oil_mon_date)
...
if st.button("Update EIA OIL MONTHLY"):
   ...
   #st.write("Natural Oil Weekly on {}".format(str(today)))
    ...
    oil_data_monthly = pd.read_csv("Data_monthly_oil/2023-10-20.csv".format(oil_mon_date_max))
